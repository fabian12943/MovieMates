class Movies::TopRated < ApplicationRecord
    self.table_name = "top_rated_movies"
    ...
    def self.update
        ...
        while updated_movies < NUMBER_MOVIES
            ...
            tmdb_map.each do |movie|
                ...
                Movies::TopRated.create(movie_tmdb_id: movie["id"])
                updated_movies += 1
            end
            page += 1
        end
    ...
    def self.tmdb_map(page, language = "en")
        tmdb_map = HTTParty.get("https://api.themoviedb.org/3/movie/top_rated?language=#{language}&page=#{page}&api_key=#{Rails.application.credentials.tmdb[:api_key]}")
        tmdb_map["results"]
    end
