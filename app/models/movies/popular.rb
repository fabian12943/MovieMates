class Movies::Popular < ApplicationRecord
    self.table_name = "popular_movies"
    ...
    def self.update
        ...
        while updated_movies < NUMBER_MOVIES
            ...
            tmdb_map.each do |movie|
                ...
                Movies::Popular.create(movie_tmdb_id: movie["id"])
                updated_movies += 1
            end
            page += 1
        end
    ...
    def self.tmdb_map(page, language = "en")
        tmdb_map = HTTParty.get("https://api.themoviedb.org/3/movie/popular?language=#{language}&page=#{page}&api_key=#{Rails.application.credentials.tmdb[:api_key]}")
        tmdb_map["results"]
    end
