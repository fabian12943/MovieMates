class UpcomingMovie < ApplicationRecord
    UPDATE_INTERVAL = 1.day

    def self.update_movies_for_all_languages
        UpcomingMovie.destroy_all

        tmdb_map = UpcomingMovie.tmdb_map(:en)["results"]
        tmdb_map.each do |movie|
            Movie.create(id: movie["id"]) if Movie.exists?(movie["id"]) == false
            UpcomingMovie.create(movie_id: movie["id"])
        end
    ...
    def self.update_details_of_movies(language_code)
        tmdb_map = UpcomingMovie.tmdb_map(language_code)
        MovieDetailSet.create_or_update_basic_details_of_movies_from_json(tmdb_map, language_code)
    end

    def self.tmdb_map(language_code)
        HTTParty.get("https://api.themoviedb.org/3/movie/upcoming?api_key=#{Rails.application.credentials.tmdb[:api_key]}&language=#{language_code.to_s}&page=1")
    end
